...
void Screen_2() { // select movement
  ...
  movementChoice.remove(0, 3);
    ...
    if (MovementSelected == 1 || MovementSelected == 2) { // balance task
      Fs = 20;
      Screen_Balance();
    } else if (MovementSelected == 3){ // squatting task..
      Fs = 50;
      Screen_Force();
    } else {
...
void Screen_Balance(){
  ...
  while ((int) (millis() - startingMillis) < (trialLength * 1000)) {
    ...
    if (state == 0) {
      ...
      pathLength += sqrt(pow((COPx - COPx_prev), 2) + pow((COPy - COPy_prev), 2));
      COPx_prev = COPx;
      COPy_prev = COPy;
    }
  }

  Time = (float) (millis() - startingMillis) / 1000;
  swayVel = pathLength / Time;

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Balance Results");
  lcd.setCursor(0, 1);
  lcd.print("Sway Velocity:");
  lcd.setCursor(0, 2);
  lcd.print((String) swayVel + " (cm/s)");
...
void Screen_Force(){
  ...
  trialLength = 30;
  state = 0;
  N = 1, AveSym = 0, PkForce = 0;
  float L_Imp = 0, R_Imp = 0;
  startingMillis = millis();
  previousMillis = millis();
  while (digitalRead(enterButton) == LOW) {
    ...
    leftForce = leftForce / count;
    rightForce = rightForce / count;
    totalForce = (leftForce + rightForce);
        
    if (totalForce > bodyweight){
      L_Imp = L_Imp + ((leftForce-L_Imp) / (N));
      R_Imp = R_Imp + ((rightForce-R_Imp) / (N));
      N++;
    }
  }  
  while (digitalRead(enterButton) == HIGH) {}
  ...
  AveSym = 200 * (R_Imp - L_Imp)/(L_Imp + R_Imp);

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Symmetry (%)"); 
  lcd.setCursor(0, 1);
  lcd.print("    " + (String) (AveSym)); 
...
void recordBodyweight() {
  ...
  leftForce = (L_TL + L_BL + L_BR + L_TR) / count;
  rightForce = (R_TL + R_BL + R_BR + R_TR) / count;
  

  if (leftForce > rightForce){
    bodyweight = leftForce;
  } else {
    bodyweight = rightForce;
  }
...
void CalibrateForcePlates() {
  ...
  L_TL_0 = -m_L_TL * ((float) L_TL) / count; 
